...
const sofiaActual=id=>{const sh=byId[id].shares;let best=null,v=-1;for(const k in sh)if(k!=="others"&&sh[k]>v){best=k;v=sh[k];}return best;};
check("Sofia map uses final leaders after the opening",g.electionNightLeader(byId["sofia-city"],.9,0)===sofiaActual("sofia-city")&&g.electionNightLeader(byId["sofia-obl"],.9,0)===sofiaActual("sofia-obl"));
const postOpeningLeaders=g.DISTRICTS.every((d,i)=>{const sh=byId[d.id].shares;let best=null,v=-1;for(const k in sh)if(k!=="others"&&sh[k]>v){best=k;v=sh[k];}return g.electionNightLeader(byId[d.id],.2,i)===best;});
check("district map uses final leaders after the opening two hours",postOpeningLeaders);
